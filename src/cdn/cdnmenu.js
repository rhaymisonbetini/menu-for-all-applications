'use strict'

let icons = getIcons()
let generalConfig = getGeneralConfig()
let css = getCss()
let state = false;
let screenWidth = window.innerWidth;
let screenHeight = window.innerHeight;
let hasRezised = false;
let applications = [];

function getIcons() {
    return {
        menurounded: "data:image/png;base64,",
        error: "data:image/png;base64,"
    }
}

function getCss() {
    return {
        circleBottom: "position:fixed; bottom:50px; right:50px; width:60px; height:60px; border:2px solid #fff; background: #fff; z-index: 99999999999999; border-radius:50%;cursor: pointer; display:flex; justify-content:center;align-items:center",
        menuNormal: "position:fixed; top:50px; width:100%; height:100px;background: #06d1d1; z-index: 99999999999999; pointer; display:flex; justify-content:center;align-items:center; opacity:0display:flex; justify-content:center;align-items:center",
        menuMobile: "position:fixed; top:50px; width:100%; min-height:100vh; height:auto; padding-top:15px; background: #06d1d1; z-index: 99999999999999; pointer; display:flex; flex-direction:column;align-items:center; opacity:0display:flex;",
        linkMenuDefault: `font-size: 24px; padding: 10px;font-weight: bold; text-decoration: none; color: #fff`,
        mouseHoverLink: `font-size: 24px; padding: 10px; color:#757575 !important`,
        mouseOutLink: `font-size: 24px; padding: 10px;font-weight: bold; text-decoration: none; color: #fff`,
        iconSizes: "width:50px; height:50px;"
    }
}

function getGeneralConfig() {
    return {
        limitWidth: 800,
        faddeInTimer: 0.00001
    }
}

async function initMenu(tokenApp, userMailToSend,applicationUrlEndpoint) {
    var token = tokenApp
    var userMail = userMailToSend
    var applicationUrl = applicationUrlEndpoint;

    if (!token || !userMail || !applicationUrl) {
        createMainMenu(false)
        return;
    }

    let menuAndConfig = await new Promise(resolve => {
        var xhr = new XMLHttpRequest();
        xhr.open("POST", applicationUrl, true);
        xhr.setRequestHeader('Content-Type', 'application/json');
        xhr.setRequestHeader('Authorization', `Bearer ${token}`);
        xhr.send(JSON.stringify({
            email: userMail
        }));
        xhr.onload = function (e) {
            resolve(xhr.response);
        };
        xhr.onerror = function () {
            resolve(undefined);
            console.error("** An error occurred during the XMLHttpRequest");
        };
    })
    if (!menuAndConfig || menuAndConfig.length <= 0) {
        createMainMenu(false)
        return;
    }
    if(typeof menuAndConfig === "string") {
        menuAndConfig = JSON.parse(menuAndConfig)
    }
    createMainMenu(menuAndConfig)

}

/**
* This method create main menu ( the circle buttom in the bottom of the page)
* @param Object data 
* @returns void
*/
function createMainMenu(data) {
    try {
        var div = document.createElement('div');
        div.id = "divID"
        div.style = css.circleBottom
        document.getElementById("q-app").appendChild(div);
        var imgIcon = document.createElement('img');
        imgIcon.style = css.iconSizes
        div.appendChild(imgIcon);

        if (data) {
            applications = data
            imgIcon.src = icons.menurounded
            div.addEventListener("click", (_) => {
                openApplicationMenu()
            })
        } else {
            imgIcon.src = icons.error
        }
    } catch (e) {
        console.log(e)
    }
}

/**
 * This method is called when the user clicked on the circle and create a menu in the top of the browsers
 * @returns void
 */
function openApplicationMenu() {
    !state ? state = true : state = false
    if (state) {
        var menu = document.createElement('div');
        menu.id = "masterMenu"
        if (screenWidth <= generalConfig.limitWidth) {
            menu.style = css.menuMobile
            applications.push({
                title: 'Close Menu',
                link: null
            })
        } else {
            menu.style = css.menuNormal
        }
        document.getElementById("q-app").appendChild(menu);
        mount()
        return;
    } else {
        closer()
    }
}

/**
* This method execute a loop in the applications array and create a link with the application in the menu on the top
* @returns void
*/
function mount() {
    for (const app of applications) {
        let body = document.getElementById("masterMenu");
        if (app.title !== 'Close Menu') {
            createLink(body, app)
        } else {
            createParagraph(body, app)
        }
    }
    show()
    window.addEventListener("resize", (_) => {
        setTimeout(() => {
            screenSizeListener()
        }, 1000)
    });
}


/**
* This method observer screenSize do create responsive menu
* @returns void
*/
function screenSizeListener() {
    var menu = document.getElementById('masterMenu');
    if (!menu) {
        return
    }
    if (window.innerWidth <= generalConfig.limitWidth) {
        menu.style = css.menuMobile
        let hasClosed = applications.findIndex(element => element.title === 'Close Menu');
        if (hasClosed == -1 && !hasRezised) {
            applications.push({
                title: 'Close Menu',
                link: null
            })
            createParagraph(menu, { title: "Close Menu" })
            hasRezised = true
        }
    } else {
        let hasClosed = applications.findIndex(element => element.title === 'Close Menu');
        if (hasClosed && hasClosed !== -1 && hasRezised) {
            applications.splice(hasClosed, 1)
            var menu = document.getElementById('masterMenu');
            menu.style = css.menuNormal
            destroiById("closeMenu")
            hasRezised = false
        }
    }
}

/**
* This method just create a link in menu
* @param HTMLElement body 
* @param Object app 
* @returns void
*/
function createLink(body, app) {
    let link = document.createElement('a');
    link.style = css.linkMenuDefault
    link.innerText = app.title
    link.href = app.link
    body.appendChild(link);
    link.addEventListener("mouseover", (_) => {
        link.style = css.mouseHoverLink
    })
    link.addEventListener("mouseout", (_) => {
        link.style = css.mouseOutLink
    })
}

/**
* This method just create Paragraph to clonse menu
* @param HTMLElement body 
* @param Object app 
* @returns void
*/
function createParagraph(body, app) {
    let paragraph = document.createElement('p');
    paragraph.style = css.linkMenuDefault
    paragraph.innerText = app.title
    paragraph.id = 'closeMenu'
    body.appendChild(paragraph);
    paragraph.addEventListener("mouseover", (_) => {
        paragraph.style = css.mouseHoverLink
    })
    paragraph.addEventListener("mouseout", (_) => {
        paragraph.style = css.mouseOutLink
    })

    paragraph.addEventListener("click", () => {
        closer()
    })
}

/**
* This method just execute a fadde in effect 
* @returns void
*/
function show() {
    var body = document.getElementById("masterMenu");
    for (var i = 0; i < 1; i += generalConfig.faddeInTimer) {
        body.style.opacity = i;
    }
}

/**
* This method just execute menu destroy
* @returns void
*/
function closer() {
    var node = document.getElementById("masterMenu");
    if (node && node.parentNode) {
        node.parentNode.removeChild(node);
        let hasClosed = applications.findIndex(element => element.title === 'Close Menu');
        if (hasClosed && hasClosed !== -1) {
            applications.splice(hasClosed, 1)
        }
        state = false;
    }
}

/**
* This method destroy htmelemt by id
* @returns void
*/
function destroiById(id) {
    var node = document.getElementById(id);
    if (node && node.parentNode) {
        node.parentNode.removeChild(node);
    }
}